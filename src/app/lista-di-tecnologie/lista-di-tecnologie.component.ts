...
export class ListaDiTecnologieComponent {
  ...
  onSubmit():void{
    ...
    for(var i=0;i<this.tecnologies.length;i++){
      this.dipendenteTecnologiaId=new DipendenteTecnologiaId(this.dipendente,this.tecnologies[i])
      this.dipendenteTecnologia=new DipendenteTecnologia(this.dipendenteTecnologiaId,this.esperienza[i],this.competenza[i])
      this.dipendentiTecnologie.push(this.dipendenteTecnologia)
    }
       

    this.currentmessage=this.tecnologiaService.salvaTutteLeCompetenze(this.dipendentiTecnologie)
