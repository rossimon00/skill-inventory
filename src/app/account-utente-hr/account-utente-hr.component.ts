...
export class AccountUtenteHrComponent {
  ...
  controlloGiustizia(){
    ...
    if(this.utenteHr.nome.length>=1
  ...
  constructor(private utenteHrService: TecnologiaService, 
  ...
  ngOnInit() {
    ...
    this.utenteHrService.trovaUtenteHr(new UtenteHREmail(this.email))
      .subscribe((value) => {
  ...
  vero(){
    if((this.confermaPassword===this.nuovaPassword) && this.nuovaPassword.length>=8){
  ...
  onSubmit() : void {
    ...
    this.utenteHrService.modificaUtenteHR(jsonDipendente).
    subscribe({
      next : (data) => {
